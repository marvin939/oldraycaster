...
#define GRID_SIZE 32	// The size of grids
#define DOT_SIZE 6
#define FPS 30
...
#define FOV (double)64			// Needs to be double!
#define PLAYER_ROT 120		// Rate of turn (rotation speed)
...
#define WALL_TEXTURES_FILE "textures32.png"
...
typedef struct RayWallIntersectStruct {
	...
	int axis;			// Which side of the block the ray intersects (MINOR/MAJOR_AXIS)
} RayWallIntersect;
...
int screenToMapIndex(Vec2D const* const pvec)
{
	int mapx, mapy;
	...
	if (pvec->x < 0 || mapx >= MAP_WIDTH)
		return -1;
	if (pvec->y < 0 || mapy >= MAP_HEIGHT)
		return -1;
...
void generateMap(char* maparray)
{
	...
	maparray[MAP_WIDTH * (MAP_HEIGHT / 2) + MAP_WIDTH / 2 + 2] = 'W';
	maparray[MAP_WIDTH * (MAP_HEIGHT - 2) + (MAP_WIDTH - 2)] = 'W';
	
	// Create bounding rectangle
	for (int row = 0; row < MAP_HEIGHT; ++row) {
		maparray[MAP_WIDTH * row] = 'W';
		maparray[MAP_WIDTH * row + MAP_WIDTH - 1] = 'W';
	}
	for (int col = 0; col < MAP_WIDTH; ++col) {
		maparray[col] = 'W';
		maparray[MAP_WIDTH * (MAP_HEIGHT - 1) + col] = 'W';
	}
}

void printMap(char const * const maparray)
{
	printf("maparray size: %d\n", MAP_SIZE);
	printf("Map contents:\n");
	for (int row = 0; row < MAP_HEIGHT; row++) {
		for (int col = 0; col < MAP_WIDTH; col++) {
...
{	
	...
	// [ Select which grid intersect to return if two intersects ]
	...
	if (hIntersected) {
		...
		rwi->axis = yoffset > 0 ? MINOR_AXIS : MAJOR_AXIS;	// minor = touches bottom, major = touches top
		rwi->mapIndex = hIndex;
		...
		rwi->axis = xoffset > 0 ? MINOR_AXIS : MAJOR_AXIS;	// minor = touches left, major = touches right
		rwi->mapIndex = vIndex;
...
int main( int argc, char* args[] )
{
	...
	SDL_Texture* const wallTextures = loadTexture(WALL_TEXTURES_FILE);
	SDL_Rect const defWallRect = {0, 0, GRID_SIZE, GRID_SIZE};	// Rectangle used to source the default wall from wallTextures.
	...
	while( !quit )
	{
		...
		double rayStartAngle = playerAngle - FOV / 2.0;	// starting ray angle (1/2*FOV clockwise of player angle)
		for (double ray = 0; ray < SCREEN_WIDTH; ++ray) {
			double rayDegrees = rayStartAngle + ray * dTheta;
			//~ printf("deg: %.2f\n", degrees);
			double rayRadians = degToRad(rayDegrees);
			Vec2D rayHeading = (Vec2D){cos(rayRadians), sin(rayRadians)};
			
			// Determine nearest horizontal and vertical intersections relative to the heading of the ray
			...
			//~ putchar('a');
			double intersectDistance = Vec2D_norm(Vec2D_subtract(intersect.location, P));
			double beta = rayDegrees - playerAngle;	// angles (degrees)
			double distance = intersectDistance * cos(degToRad(beta));
			double renderHeight = WALL_HEIGHT * GRID_SIZE / distance;
			//~ renderHeight = min(renderHeight, SCREEN_HEIGHT);	// So we keep drawing rays if we're distance=0 from wall.
																// (Most importantly, SDL draws the lines with y<0)
			
			//~ int shade;	// the further away, the darker the colour of the ray
			//~ shade = (double)255 * GRID_SIZE / distance;
			...
			//~ double bottomY = topY + renderHeight;
			//~ SDL_RenderDrawLine(gRenderer, ray, topY, ray, bottomY);
			
			
			// Draw texture instead of line.
			int c;	// Column of texture to draw
			
			// Select which column of the texture to draw based on where the ray is intersecting the wall
			if (intersect.type == HORIZONTAL_INTERSECTION) {
				int r = (int)intersect.location.x % GRID_SIZE;
				if (intersect.axis == MAJOR_AXIS) {
					c = r;
				}
				else {
					c = GRID_SIZE - r - 1;
			...
			else {	// VERTICAL_INTERSECTION
				int r = (int)intersect.location.y % GRID_SIZE;
				if (intersect.axis == MAJOR_AXIS) {
					c = GRID_SIZE - r - 1;
				}
				else {
			...
			SDL_Rect srcRect = {c, 0, 1, GRID_SIZE};	// 1 column of wall texture
			SDL_Rect dstRect = {ray, topY, 1, renderHeight};
			SDL_RenderCopy(gRenderer, wallTextures, &srcRect, &dstRect);
		}
		//~ return 0; // <<==================================TEMP STOP
		...
		for (int row = 0; row < MAP_HEIGHT; ++row) {
			for (int col = 0; col < MAP_WIDTH; ++col) {
				...
				if (maparray[bi] == 'W') {
					...
					SDL_RenderCopy(gRenderer, wallTextures, &defWallRect, &bc);
				}
			}
		}
		...
		// Draw a slice of the default wall texture, scaled.
		//~ int sc = 3;
		//~ SDL_Rect dstRect = {0,
							//~ SCREEN_HEIGHT - GRID_SIZE * sc, 
							//~ GRID_SIZE * sc, 
							//~ GRID_SIZE * sc};
		//~ SDL_RenderCopy(gRenderer, wallTextures, &defWallRect, &dstRect);
	...
	SDL_DestroyTexture(wallTextures);
